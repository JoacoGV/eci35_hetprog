...
// SystemVerilog created from k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE1_0000b_function_cra_agent
...
module k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE1_0000b_function_cra_agent (
    ...
    wire [3:0] finish_counter_addr_q;
    ...
    wire [3:0] dupName_5_address_ref_x_q;
    ...
    wire [3:0] dupName_6_address_ref_x_q;
    ...
    wire [3:0] dupName_7_address_ref_x_q;
    ...
    wire [3:0] dupName_8_address_ref_x_q;
    ...
    wire [3:0] dupName_9_address_ref_x_q;
    ...
    wire [3:0] dupName_10_address_ref_x_q;
    ...
    wire [3:0] dupName_11_address_ref_x_q;
    ...
    wire [3:0] dupName_12_address_ref_x_q;
    ...
    wire [3:0] dupName_13_address_ref_x_q;
    ...
    // printf_bit(BITSELECT,133)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,134)
    ...
    // clear_to_zero(CONSTANT,20)
    assign clear_to_zero_q = $unsigned(2'b00);

    // last_finish_state(REG,87)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // not_last_finish_state(LOGICAL,100)
    ...
    // readdata_valid_stage1_reg(REG,141)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    assign finish_counter_addr_q = $unsigned(4'b0101);

    // is_finish_counter_addr(LOGICAL,84)
    assign is_finish_counter_addr_q = $unsigned(avs_cra_address == finish_counter_addr_q ? 1'b1 : 1'b0);

    // cra_output_was_finish_counter_address_stage1_reg(REG,32)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // start_bit_computation(LOGICAL,155)
    assign start_bit_computation_q = not_start_q & start_bit_b;

    // status_self_update(BITJOIN,168)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,33)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,30)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,111)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,142)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,31)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,53)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,47)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,55)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,49)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,57)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,51)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,62)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,64)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,66)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,174)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
