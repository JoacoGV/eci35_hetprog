...
// SystemVerilog created from k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE_c0000e_function_cra_agent
...
module k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE_c0000e_function_cra_agent (
    ...
    output wire [511:0] kernel_arguments,
    ...
    wire [511:0] kernel_arg_bit_join_q;
    ...
    reg [511:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_start_q;
    reg [511:0] pipe_reg1_kernel_arguments_q;
    ...
    // printf_bit(BITSELECT,171)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,172)
    ...
    // readdata_valid_stage1_reg(REG,179)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,71)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,66)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,63)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,146)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,180)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,64)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,87)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,81)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,89)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,83)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,91)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,85)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,96)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,98)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,100)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // pipe_reg0_kernel_arguments(REG,167)
    always @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,169)
    always @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,212)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
